...
include( 'colorsofimage.class.php' );
...
add_action('admin_head', function(){
    echo	'<style>'.
			'span.color{width:2em; height:2em; margin-right:.5em; display:inline-block; border-radius:2px; border:1px Solid rgba(0,0,0,0.1)}'.
...
function attachment_palette($id){
...
function attachment_hex_average($array){
...
function attachment_hex_array($src){
...
add_filter( 'attachment_fields_to_edit', function( $form_fields, $post ){
    ...
    $colors = attachment_palette($post->ID);
...
add_filter( 'attachment_fields_to_save', function( $post, $attachment ){
    ...
    global $set;
        ...
        $colors = attachment_hex_array($src);
    	
        $tone = (attachment_hex_average($colors) > (255 / 2)) ? true : false;
...
add_action('admin_menu', function () {
    add_options_page(
    __("Media parser", "libraries"),
    __("Media parser", "libraries"),
    'manage_options',
    'media-colors',
        function () {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'media-colors'));
        }
